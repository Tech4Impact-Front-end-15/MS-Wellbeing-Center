...
let getDataDummy = async () => {
  ...
  for (let y = 0; y < 1; y++) {
    dummyContainer.innerHTML += `
      ...
      <div>
        ...
        <div class="font-serif px-5 italic text-center">Author by : ${dummy[5].author}</div>
        <div class="flex flex-wrap justify-center w-full">
        <img src="${dummy[5].img}" alt="" srcset=""/>
        </div>
  ...
  for (let i = 0; i < dummy[6].subtittle.length; i++) {
    contentContainer.innerHTML += `
    <div class="font-serif px-5 pt-5 text-xl font-bold text-textPrimary">${dummy[5].subtittle[i]}
          </div>
          <div class="font-serif px-5 text-justify">
        <p class="pt-5 indent-8">
          ${dummy[5].descContent[i]}
        </p>
    </div>`;
  }
